...
# -----------------------------------------------------------
# Mostrar la matriz como texto legible
# -----------------------------------------------------------
def mostrar_matriz_textual(titulo, matriz):
    texto = f"{titulo}\n"
    texto += "    " + " ".join([f"{i+1:2}" for i in range(len(matriz))]) + "\n"
    for i, fila in enumerate(matriz):
        texto += f"{i+1:2} | " + " ".join([f"{v:2}" for v in fila]) + "\n"
    return texto + "\n"
...
def generar_y_mostrar_grafo(root, n, modo):
    ...
    pasos.append(mostrar_matriz_textual("Matriz ordenada por filas y columnas:", matriz_ordenada))
